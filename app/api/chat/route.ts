...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = [
      ...
      "Answer only what the user asked. Do not add unnecessary background, explanations, summaries, or repeated information.",
      "For greetings or simple conversational messages, reply naturally in one short sentence.",
      "Ask for missing project inputs ONE AT A TIME, using one short question only.",
      "Never send a long paragraph unless the user explicitly asks for a detailed explanation.",
